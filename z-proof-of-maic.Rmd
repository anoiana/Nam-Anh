...
(\#eq:eq5)
$$
differentiating both sides of \@ref(eq:eq5) w.r.t $\alpha_1$

$$
\sum_{i=1}^n\bigg(p_iz_i\sum_{j=1}^n\Big[\exp[\alpha_1z_j]+constant\Big]\bigg) = \sum_{i=1}^n\exp (\alpha_1z_i)> 0.
$$
...
where $k = \sum_{j=1}^n\big[\exp[\alpha_1z_j]+constant\big]$. Thus, the condition \@ref(eq:eq6) is satisfied as the equation \@ref(eq:eq7) equals zero, which is equivalent to minimizing the right hand side of \@ref(eq:eq5), $\sum_{i=1}^n\exp (\alpha_1z_i)\ge 0$, which can be rewritten in the matrix form when there are more than one effect modifiers 
